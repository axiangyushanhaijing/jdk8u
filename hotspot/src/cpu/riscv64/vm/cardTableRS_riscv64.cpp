...
jbyte CardTableRSRv::find_unused_youngergenP_card_value() {
...
void CardTableRSRv::prepare_for_younger_refs_iterate(bool parallel) {
...
/*void CardTableRSRv::younger_refs_iterate(Generation* g,
...
inline bool ClearNoncleanCardWrapper::clear_card_parallel(jbyte* entry) {
  while (true) {
    ...
    assert(entry_val != CardTableRSRv::clean_card_val(),
           "We shouldn't be looking at clean cards, and this should "
           "be the only place they get cleaned.");
    if (CardTableRSRv::card_is_dirty_wrt_gen_iter(entry_val)
        || _ct->is_prev_youngergen_card_val(entry_val)) {
      jbyte res =
        Atomic::cmpxchg(CardTableRSRv::clean_card_val(), entry, entry_val);
      if (res == entry_val) {
        break;
      } else {
        assert(res == CardTableRSRv::cur_youngergen_and_prev_nonclean_card,
               "The CAS above should only fail if another thread did "
               "a GC write barrier.");
      }
    } else if (entry_val ==
               CardTableRSRv::cur_youngergen_and_prev_nonclean_card) {
...
inline bool ClearNoncleanCardWrapper::clear_card_serial(jbyte* entry) {
  ...
  assert(entry_val != CardTableRSRv::clean_card_val(),
         "We shouldn't be looking at clean cards, and this should "
         "be the only place they get cleaned.");
  assert(entry_val != CardTableRSRv::cur_youngergen_and_prev_nonclean_card,
         "This should be possible in the sequential case.");
  *entry = CardTableRSRv::clean_card_val();
  return true;
}

ClearNoncleanCardWrapper::ClearNoncleanCardWrapper(
  DirtyCardToOopClosure* dirty_card_closure, CardTableRSRv* ct, bool is_par) :
...
void ClearNoncleanCardWrapper::do_MemRegion(MemRegion mr) {
  ...
  while (cur_entry >= limit) {
    ...
    if ((*cur_entry != CardTableRSRv::clean_card_val()) && clear_card(cur_entry)) {
      ...
      if (is_word_aligned(cur_entry)) {
        ...
        while (cur_row >= limit && *((intptr_t*)cur_row) ==  CardTableRSRv::clean_card_row_val()) {
...
void CardTableRSRv::write_ref_field_gc_par(void* field, oop new_val) {
...
/*void CardTableRSRv::younger_refs_in_space_iterate(Space* sp,
                                                OopsInGenClosure* cl,
                                                uint n_threads) {
...
#ifdef ASSERT
void CardTableRSRv::verify_used_region_at_save_marks(Space* sp) const {
...
void CardTableRSRv::clear_into_younger(Generation* old_gen) {
...
void CardTableRSRv::invalidate_or_clear(Generation* old_gen) {
 assert(old_gen->level() == 1, "Should only be called for the old generation");
...
class VerifyCTSpaceClosureRv: public SpaceClosure {
private:
  CardTableRSRv* _ct;
  HeapWord* _boundary;
public:
  VerifyCTSpaceClosureRv(CardTableRSRv* ct, HeapWord* boundary) :
...
public:
  VerifyCleanCardClosureRv(HeapWord* b, HeapWord* begin, HeapWord* end) :
  ...
  virtual void do_oop(oop* p)       { VerifyCleanCardClosureRv::do_oop_work(p); }
  virtual void do_oop(narrowOop* p) { VerifyCleanCardClosureRv::do_oop_work(p); }
};



void CardTableRSRv::verify_space(Space* s, HeapWord* gen_boundary) {
  ...
  while (cur_entry < limit) {
    if (*cur_entry == clean_card_val()) {
      ...
      if (begin < end) {
        ...
        VerifyCleanCardClosureRv verify_blk(gen_boundary, begin, end);
...
CardTableRSRv::CardTableRSRv(MemRegion whole_heap, bool scanned_concurrently) :
...
CardTableRSRv::~CardTableRSRv() {
  if (_last_cur_val_in_gen) {
    FREE_C_HEAP_ARRAY_RV(jbyte, _last_cur_val_in_gen);
    _last_cur_val_in_gen = NULL;
  }
  if (_lowest_non_clean) {
    FREE_C_HEAP_ARRAY_RV(CardArr, _lowest_non_clean);
    _lowest_non_clean = NULL;
  }
  if (_lowest_non_clean_chunk_size) {
    FREE_C_HEAP_ARRAY_RV(size_t, _lowest_non_clean_chunk_size);
    _lowest_non_clean_chunk_size = NULL;
  }
  if (_lowest_non_clean_base_chunk_index) {
    FREE_C_HEAP_ARRAY_RV(uintptr_t, _lowest_non_clean_base_chunk_index);
    _lowest_non_clean_base_chunk_index = NULL;
  }
  if (_last_LNC_resizing_collection) {
    FREE_C_HEAP_ARRAY_RV(int, _last_LNC_resizing_collection);
    _last_LNC_resizing_collection = NULL;
  }
}

void CardTableRSRv::initialize() {
...
bool CardTableRSRv::card_will_be_scanned(jbyte cv) {
  return card_is_dirty_wrt_gen_iter(cv) || is_prev_nonclean_card_val(cv);
}

bool CardTableRSRv::card_may_have_been_dirty(jbyte cv) {
  return
    cv != clean_card &&
    (card_is_dirty_wrt_gen_iter(cv) ||
     CardTableRSRv::youngergen_may_have_been_dirty(cv));
}

/*void CardTableRSRv::non_clean_card_iterate_possibly_parallel(
  Space* sp,
  MemRegion mr,
  OopsInGenClosure* cl,
  CardTableRSRv* ct,
...
void CardTableRSRv::non_clean_card_iterate_parallel_work(Space* sp, MemRegion mr,
                                                       OopsInGenClosure* cl, CardTableRSRv* ct,
                                                       uint n_threads) {
  fatal("Parallel gc not supported here.");
}

bool CardTableRSRv::is_in_young(oop obj) const {
