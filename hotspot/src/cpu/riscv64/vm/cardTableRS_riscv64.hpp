...
class CardTableRSRv: public CardTable {
  friend class VMStructs;
  // Below are private classes used in impl.
  friend class VerifyCTSpaceClosureRv;
  ...
  // Number of generations, plus one for lingering PermGen issues in CardTableRSRv.
...
public:
  CardTableRSRv(MemRegion whole_heap, bool scanned_concurrently);
  ~CardTableRSRv();
  ...
  static bool youngergen_may_have_been_dirty(jbyte cv) {
    return cv == CardTableRSRv::cur_youngergen_and_prev_nonclean_card;
  }
...
public:
  ClearNoncleanCardWrapper(DirtyCardToOopClosure* dirty_card_closure, CardTableRSRv* ct, bool is_par);
